...
namespace Armincms\Categorizable\Menus;

use Armincms\Categorizable\Cypress\Fragments\Category as Fragment;
use Armincms\Contract\Menus\MenuItemType; 
use Zareismail\Gutenberg\Gutenberg;
use Laravel\Nova\Fields\Select;
use Laravel\Nova\Http\Requests\NovaRequest; 

class Category extends MenuItemType
{  
	/**
	...
	{
		return \Armincms\Categorizable\Nova\Category::class;
	}
	    ...
	    return array_merge([
	    	Select::make(__('Target Fragment'), 'fragment')->options(function() {
	    			return Gutenberg::cachedFragments()
	    					->where('fragment', Fragment::class)
	    					->keyBy->getKey()->map->name;
	    		})
	    		->required()
	    		->rules('required'), 
	    ], parent::getFields());
	...
	{    
		...
		if (is_null($page = static::findCategory($value))) {
			return $value;
		} 
	 ...
	 */
	public static function findCategory($key)
	{
